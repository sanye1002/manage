                           ...
                           <#-- <div style="float:right;margin-right:2px;">
                            ...
                            <div class="table-scrollable">
                                ...
                                <tr id="${p.getPersonnelSalaryAdvance().getId()}"
                                    ...
                                    <#if p.getPersonnelSalaryAdvance().getBackStatus()==0>class="warning" </#if>
                                >
                                    <td>${p.getPersonnelSalaryAdvance().getMonth()}</td>
                                    ...
                                    <td>
                                            ...
                                            <#if p.getPersonnelSalaryAdvance().getBackStatus()==1>
                                                已归还
                                            </#if>
                                    </td>
                                    <td>${p.getPersonnelSalaryAdvance().getBackTime()!}</td>
                                    ...
                                    <td>
                                        <#if p.getPersonnelSalaryAdvance().getCheckStatus()==0>
                                             <a class="btn btn-info btn-xs" onclick="grants(${p.getPersonnelSalaryAdvance().getId()},1)"><i class="fa fa-edit"></i> 拨款
                                             </a>
                                            <a class="btn btn-danger btn-xs edit" onclick="grants(${p.getPersonnelSalaryAdvance().getId()},0)"><i class="fa fa-frown-o"></i>
                                                撤回</a>
                                        <#else >
                                        ...
                                        <a class="btn btn-yellow btn-xs shiny"
                                           onclick="showImg(${p.getPersonnelSalaryAdvance().id})"><i
...
<script>
    ...
    function back(id) {
        layer.confirm('是否确定已归还？', {
                    ...
                    //执行POST请求
            $.post(
                    "/oa/personnelSalaryAdvance/back",
                    {
                        id: id
                    },
                    function (res) {
    ...
    function grants(id, resultStatus) {
        layer.prompt({title: '输入备注，并确认', formType: 2}, function (remark, index) {
            if (remark == "") {
                ...
                layer.close(index);
                $.post(
                        "/oa/personnelSalaryAdvance/grants",
                        {
                            id: id,
                            remark: remark,
    ...
    $(function () {
        ...
        $("#fa-search").click(function () {
            var month = $("#month").val();
            if (month != "") {
                location = "/oa/personnelSalaryAdvance/checkList.html?month=" + month
            }
        })
        $("input[type=checkbox]").click(function () {
            var resultStatus = $(this).val();
            var month = $("#month").val();
            location = "/oa/personnelSalaryAdvance/checkList?month=" + month + "&checkStatus=1&resultStatus=" + resultStatus
