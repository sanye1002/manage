                           ...
                           <#-- <div style="float:right;margin-right:2px;">
                            ...
                            <div style="float:right;margin-right:2px;">
                                ...
                                <div class="form-group">
                                    <select id="selectType">
                                        <option value="1" <#if checkStatus==1>selected</#if> >已出库 <span
                            ...
                            <table class="table table-bordered table-hover">
                                ...
                                <tr id="${p.getItemDebit().getId()}"
                                    ...
                                    <#if p.getItemDebit().getBackStatus()==0>class="warning" </#if>
                                >
                                    <td>${p.getItemDebit().getItemName()!}</td>
                                    <td>${p.getUserInfo().getName()}</td>
                                    <td>${p.getItemDebit().getCreateTime()}</td>
                                    <td>${p.getItemDebit().getSumAmount()}</td>
                                    <td style="max-width: 400px">${p.getItemDebit().getRemark()!}</td>
                                    <td>
                                            <#if p.getItemDebit().getBackStatus()==0>
                                                未归还
                                            </#if>
                                            <#if p.getItemDebit().getBackStatus()==1>
                                                已归还
                                            </#if>
                                    </td>
                                    <td>${p.getItemDebit().getBackTime()!}</td>
                                    <td>
                                    ...
                                    <td>
                                        <#if p.getItemDebit().getCheckStatus()==0>
                                             <a class="btn btn-info btn-xs" onclick="grants(${p.getItemDebit().getId()},1)"><i class="fa fa-edit"></i> 出库
                                             </a>
                                            <a class="btn btn-danger btn-xs edit" onclick="grants(${p.getItemDebit().getId()},0)"><i class="fa fa-frown-o"></i>
                                                撤回</a>
                                        <#else >
                                        ...
                                        <a class="btn btn-danger btn-xs"><i class="fa fa-times"></i>
                                            删除</a>
                                        <#if p.getItemDebit().getBackStatus()==0>
                                             <a class="btn btn-info btn-xs" onclick="back(${p.getItemDebit().getId()})"><i class="fa fa-edit"></i> 归还
                                             </a>
                                        </#if>
                                    </td>
                                </tr>
...
<script>
    ...
    $(function () {
        ...
        $("input[type=checkbox]").click(function () {
            ...
            location = "/oa/itemDebit/list.html?&checkStatus=1&resultStatus=" + resultStatus
        ...
        $("#selectType").change(function () {
            ...
            location = "/oa/itemDebit/list.html?&resultStatus=" + resultStatus + "&checkStatus=" + checkStatus
        })
...
<script>
    ...
    function showImg(id) {
        $.getJSON('/layer/personnelSalaryAdvance/' + id, function (json) {
            layer.photos({
                photos: json
