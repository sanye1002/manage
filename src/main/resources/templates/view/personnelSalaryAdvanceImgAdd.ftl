...
<script>
    var flog = true;
    var id =  ${personnelSalaryAdvance.getId()};
    $(function () {
        $("#delete").click(function () {
            layer.confirm('重置图片上传？', {
                        ...
                        //执行POST请求
                        $.post(
                                "/oa/personnelSalaryAdvance/remove/img",
                                {
                                    id: id
                                },
                                function (res) {
        ...
        $("#submit").click(function () {
            if (flog) {
                $.post(
                        "/oa/personnelSalaryAdvance/check",
                        {id:id},
                        function (res) {
                            layer.msg(res.message, {
                                time: 1000
                            ...
                            if (res.code == 0) {
                                setTimeout(function () {
                                    location = "/oa/personnelSalaryAdvance/index.html"
                                }, 1000)
                            }
                            //如果上传失败
                            if (res.code > 0) {
        ...
        layui.use('upload', function () {
            ...
            upload.render({
                elem: '#image-select'
                , url: '/upload/imgs/personnelSalaryAdvance/${personnelSalaryAdvance.getId()}'
                , auto: false
                , multiple: true
                , acceptMime: 'image'
